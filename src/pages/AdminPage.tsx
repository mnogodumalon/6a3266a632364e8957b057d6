...
const MANGEL_FIELDS = [
  ...
  { key: 'status', label: 'Status', type: 'lookup/select', options: [{ key: 'offen', label: 'Offen' }, { key: 'in_bearbeitung', label: 'In Bearbeitung' }, { key: 'behoben', label: 'Behoben' }] },
  { key: 'frist', label: 'Frist', type: 'date/date' },
...
export default function AdminPage() {
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'mangel':
        lists.baustelleList = (data as any).baustelle ?? [];
        break;
      case 'bericht':
        lists.baustelleList = (data as any).baustelle ?? [];
        break;
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'mangel' && fieldKey === 'baustelle') {
      const match = (lists.baustelleList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.name ?? '—';
    }
    if (entity === 'bericht' && fieldKey === 'baustelle') {
      const match = (lists.baustelleList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.name ?? '—';
    }
    return String(url);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'mangel': return {
        ...
        remove: (id: string) => LivingAppsService.deleteMangelEntry(id),
      };
      case 'bericht': return {
    ...
    >
      ...
      {(createEntity === 'mangel' || dialogState?.entity === 'mangel') && (
        <MangelDialog
          ...
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'mangel' ? handleUpdate : (fields: any) => handleCreate('mangel', fields)}
          defaultValues={dialogState?.entity === 'mangel' ? dialogState.record?.fields : undefined}
          baustelleList={(data as any).baustelle ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Mangel']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Mangel']}
        />
      )}
      {(createEntity === 'bericht' || dialogState?.entity === 'bericht') && (
        <BerichtDialog
          open={createEntity === 'bericht' || dialogState?.entity === 'bericht'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'bericht' ? handleUpdate : (fields: any) => handleCreate('bericht', fields)}
          defaultValues={dialogState?.entity === 'bericht' ? dialogState.record?.fields : undefined}
          baustelleList={(data as any).baustelle ?? []}
      ...
      {viewState?.entity === 'mangel' && (
        <MangelViewDialog
          open={viewState?.entity === 'mangel'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'mangel', record: r }); }}
          baustelleList={(data as any).baustelle ?? []}
        />
      )}
      {viewState?.entity === 'bericht' && (
        <BerichtViewDialog
          open={viewState?.entity === 'bericht'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
