...
import type { Mangel, Baustelle } from '@/types/app';
...
export default function MangelPage() {
  ...
  const [records, setRecords] = useState<Mangel[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Mangel | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Mangel | null>(null);
  const [sortKey, setSortKey] = useState('');
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('asc');
  const [baustelleList, setBaustelleList] = useState<Baustelle[]>([]);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, baustelleData] = await Promise.all([
        LivingAppsService.getMangel(),
        LivingAppsService.getBaustelle(),
      ]);
      setRecords(mainData);
      setBaustelleList(baustelleData);
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Mangel['fields']) {
    await LivingAppsService.createMangelEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Mangel['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateMangelEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteMangelEntry(deleteTarget.record_id);
    setRecords(prev => prev.filter(r => r.record_id !== deleteTarget.record_id));
    setDeleteTarget(null);
  }

  function getBaustelleDisplayName(url?: unknown) {
    if (!url) return '—';
    const id = extractRecordId(url);
    return baustelleList.find(r => r.record_id === id)?.fields.name ?? '—';
  }
  ...
  return (
    <PageShell
      title="Mangel"
      subtitle={`${records.length} Mangel im System`}
      action={
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('frist')}>
                <span className="inline-flex items-center gap-1">
                  Frist
                  {sortKey === 'frist' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('baustelle')}>
                <span className="inline-flex items-center gap-1">
                  Baustelle
                  {sortKey === 'baustelle' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/mangel/${record.record_id}`); }}>
                ...
                <TableCell className="text-muted-foreground">{formatDate(record.fields.frist)}</TableCell>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getBaustelleDisplayName(record.fields.baustelle)}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Mangel. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
