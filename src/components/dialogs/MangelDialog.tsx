...
import type { Mangel, Baustelle, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Mangel';
...
interface MangelDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Mangel['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Mangel['fields'], 'status'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status: LOOKUP_OPTIONS['mangel']?.['status'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  baustelle: APP_IDS.BAUSTELLE,
};
...
export function MangelDialog({ open, onClose, onSubmit, defaultValues, recordId, baustelleList, enablePhotoScan = true, enablePhotoLocation = true }: MangelDialogProps) {
  const [fields, setFields] = useState<Partial<Mangel['fields']>>({});
  ...
  // Inline-Create state for "Baustelle" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraBaustelle` list, and select it in
  // the originating Combobox via the captured `createBaustelleField`.
  const [createBaustelleOpen, setCreateBaustelleOpen] = useState(false);
  const [createBaustelleInitial, setCreateBaustelleInitial] = useState('');
  const [createBaustelleField, setCreateBaustelleField] = useState<string>('');
  const [extraBaustelle, setExtraBaustelle] = useState< Baustelle[]>([]);
  const baustelleListAll = useMemo(
    () => [...baustelleList, ...extraBaustelle],
    [baustelleList, extraBaustelle],
  );
  function openCreateBaustelle(fieldKey: string, q: string) {
    setCreateBaustelleField(fieldKey);
    setCreateBaustelleInitial(q);
    setCreateBaustelleOpen(true);
  }
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Mangel['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'mangel');
      await onSubmit(clean as Mangel['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="baustelle" entity="Baustelle">\n${JSON.stringify(baustelleList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      setFields(prev => {
        ...
        if (baustelleName) {
          const baustelleMatch = baustelleList.find(r => matchName(baustelleName!, [String(r.fields.name ?? '')]));
          if (baustelleMatch) merged['baustelle'] = createRecordUrl(APP_IDS.BAUSTELLE, baustelleMatch.record_id);
        }
        return merged as Partial<Mangel['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues ? 'Mangel bearbeiten' : 'Mangel hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'frist': (
      <div key="frist" className="space-y-1.5">
    ...
    'baustelle': (
      <div key="baustelle" className="space-y-1.5">
        ...
        <Combobox
          ...
          onChange={id => setFields(f => ({ ...f, baustelle: id ? createRecordUrl(APP_IDS.BAUSTELLE, id) : undefined }))}
          searchPlaceholder="Suchen…"
          emptyText="Kein Treffer"
          onCreateNew={(q) => openCreateBaustelle("baustelle", q)}
          createLabel="Neu in Baustelle"
        />
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"titel": "Titel", "beschreibung": "Beschreibung", "status": "Status", "frist": "Frist", "foto": "Foto", "baustelle": "Baustelle"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"baustelle": {"name": "Name", "adresse": "Adresse", "bauleiter": "Bauleiter", "status": "Status", "titelfoto": "Titelfoto"}};
  ...
  return (
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.MANGEL} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createBaustelleOpen && (
      <BaustelleDialog
        open={createBaustelleOpen}
        onClose={() => setCreateBaustelleOpen(false)}
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createBaustelleEntry(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as Baustelle;
            setExtraBaustelle(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.BAUSTELLE, result.id);
            setFields(prev => ({ ...prev, [createBaustelleField]: url } as any));
          }
          setCreateBaustelleOpen(false);
        }}
        defaultValues={createBaustelleInitial
          ? ({ name: createBaustelleInitial } as any)
          : undefined}
      />
    )}
    </>
