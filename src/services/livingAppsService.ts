...
import type { Baustelle, Mangel, Bericht, CreateBaustelle, CreateMangel, CreateBericht } from '@/types/app';
...
export class LivingAppsService {
  // --- BAUSTELLE ---
  static async getBaustelle(): Promise<Baustelle[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BAUSTELLE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Baustelle[];
    return enrichLookupFields(records, 'baustelle');
  }
  static async getBaustelleEntry(id: string): Promise<Baustelle | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BAUSTELLE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Baustelle;
    return enrichLookupFields([record], 'baustelle')[0];
  }
  static async createBaustelleEntry(fields: CreateBaustelle) {
    return callApi('POST', `/apps/${APP_IDS.BAUSTELLE}/records`, { fields: cleanFieldsForApi(fields as any, 'baustelle') });
  }
  static async updateBaustelleEntry(id: string, fields: Partial<CreateBaustelle>) {
    return callApi('PATCH', `/apps/${APP_IDS.BAUSTELLE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'baustelle') });
  }
  static async deleteBaustelleEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BAUSTELLE}/records/${id}`);
  }

  // --- MANGEL ---
  static async getMangel(): Promise<Mangel[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MANGEL}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Mangel[];
    return enrichLookupFields(records, 'mangel');
  }
  static async getMangelEntry(id: string): Promise<Mangel | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MANGEL}/records/${id}`);
    const record = { record_id: data.id, ...data } as Mangel;
    return enrichLookupFields([record], 'mangel')[0];
  }
  static async createMangelEntry(fields: CreateMangel) {
    return callApi('POST', `/apps/${APP_IDS.MANGEL}/records`, { fields: cleanFieldsForApi(fields as any, 'mangel') });
  }
  static async updateMangelEntry(id: string, fields: Partial<CreateMangel>) {
    return callApi('PATCH', `/apps/${APP_IDS.MANGEL}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'mangel') });
  }
  static async deleteMangelEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.MANGEL}/records/${id}`);
  }

  // --- BERICHT ---
  static async getBericht(): Promise<Bericht[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BERICHT}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Bericht[];
    return enrichLookupFields(records, 'bericht');
  }
  static async getBerichtEntry(id: string): Promise<Bericht | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BERICHT}/records/${id}`);
    const record = { record_id: data.id, ...data } as Bericht;
    return enrichLookupFields([record], 'bericht')[0];
  }
  static async createBerichtEntry(fields: CreateBericht) {
    return callApi('POST', `/apps/${APP_IDS.BERICHT}/records`, { fields: cleanFieldsForApi(fields as any, 'bericht') });
  }
  static async updateBerichtEntry(id: string, fields: Partial<CreateBericht>) {
    return callApi('PATCH', `/apps/${APP_IDS.BERICHT}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'bericht') });
  }
  static async deleteBerichtEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BERICHT}/records/${id}`);
  }
