...
import type { Baustelle, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Baustelle';
...
interface BaustelleDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Baustelle['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Baustelle['fields'], 'status'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status: LOOKUP_OPTIONS['baustelle']?.['status'] ?? [],
};
...
export function BaustelleDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: BaustelleDialogProps) {
  const [fields, setFields] = useState<Partial<Baustelle['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Baustelle['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'baustelle');
      await onSubmit(clean as Baustelle['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "name": string | null, // Name\n  "adresse": string | null, // Adresse\n  "bauleiter": string | null, // Bauleiter\n  "status": LookupValue | null, // Status (select one key: "in_planung" | "aktiv" | "abgeschlossen") mapping: in_planung=In Planung, aktiv=Aktiv, abgeschlossen=Abgeschlossen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Baustelle['fields']>;
      });
      ...
      if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
        try {
          ...
          setFields(prev => ({ ...prev, titelfoto: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Baustelle bearbeiten' : 'Baustelle hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'adresse': (
      <div key="adresse" className="space-y-1.5">
    ...
    'bauleiter': (
      <div key="bauleiter" className="space-y-1.5">
    ...
    'titelfoto': (
      <div key="titelfoto" className="space-y-1.5">
        <Label htmlFor="titelfoto">Titelfoto</Label>
        {fields.titelfoto ? (
          <div className="flex items-center gap-3 rounded-lg border p-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm truncate text-foreground">{fields.titelfoto.split("/").pop()}</p>
              <div className="flex gap-2 mt-1">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setFields(f => ({ ...f, titelfoto: fileUrl }));
                ...
                <button
                  ...
                  onClick={() => setFields(f => ({ ...f, titelfoto: undefined }))}
                >
                  Entfernen
                </button>
              </div>
  ...
  const FIELD_LABELS: Record<string, string> = {"name": "Name", "adresse": "Adresse", "bauleiter": "Bauleiter", "status": "Status", "titelfoto": "Titelfoto"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.BAUSTELLE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
